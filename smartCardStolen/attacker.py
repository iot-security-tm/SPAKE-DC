...
print("[+] Waiting to capture M2 (Z1, T1) and M3 (Z8, Z9, Z10, T3): ", end="", flush=True)
sock.listen(1)
...
# recv M2
Z1 , _ , _ , _ , _ , _ , T1 , _= cs.recv(10240).decode("utf-8").split(",")
Z1 = from_b64(Z1)

# recv M3 
Z8, Z9, Z10, _, T3 = cs.recv(10240).decode("utf-8").split(",")
Z8 = from_b64(Z8)
Z9 = from_b64(Z9)
Z10 = from_b64(Z10)
...
print(f"[+] PID_new: {to_b64(PIDu_new)}")


# SK
n1 = xor_bytes( [ h(to_b64(JIDu) + T1).encode(), Z1]).decode("utf-8")
n2 = xor_bytes([ h(IDd + T3).encode(), Z9 ]).decode("utf-8")
