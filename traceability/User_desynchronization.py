...
for i in range(4):
    ...
    a = xor_bytes([ PIDu_new.encode() , "1234567890".encode() ])
    Z3 = to_b64(xor_bytes([h(n1 + to_b64(JIDu) + to_b64(Nu) + T1).encode(), a]))
    R1 = h(to_b64(JIDu) + IDd + n1 + T1)

    M1 = f"{Z1},{Z2},{Z3},{R1},{PIDu},{T1}"
    
    print(f"[*] M1 has been sent.")
    time.sleep(1)
    sock.send(M1.encode())
    ...
    if M3 == b"[-] User Not Found!":
         print("[-] Authentication failed.")
         continue
    
    print(f"[*] M3 has been received.")
    time.sleep(1)

    Z8, Z9, Z10, R3, T3 = M3.decode("utf-8").split(",")
    Z8 = from_b64(Z8)
    Z9 = from_b64(Z9)
    Z10 = from_b64(Z10)
