...
with open("PID","w") as f:
     f.write(f"{to_b64(PIDu.encode())}: {to_b64(JIDu_star)},{kd}")
...
for i in range(4):
    ...
    with open("PID", "r") as f:
        db = f.read()
        stored_PID = db.split(":")[0]

        if stored_PID != to_b64(PIDu.encode()):
            user.send(b"[-] User Not Found!")
            print("[-] User Not Found!")
            print(f"[!] Contorling Server's Data Base (PIDu: JIDu_start,kd): {db}")
            continue
    ...
    print(f"[*] M2 has been sent.")
    time.sleep(1)
    drone.send(M2.encode())


    # sending m3 from D to U
    user.send(drone.recv(10240))
    ...
    with open("PID","w") as f:
        f.write(f"{to_b64(PIDu_new)}: {to_b64(JIDu_star)},{kd}")